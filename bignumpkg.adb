...
-- MODIFIED BY CHRIS RAND: 4/6/2018
...
package body BigNumPkg is
   ...
   ----------------------------------------------------------
   -- Purpose: Converts bignum to string without leading 0s
   -- Parameters: X: bignum to convert
   -- Precondition: X is a valid bignum
   -- Postcondition: returns the string representation of X
   ----------------------------------------------------------
   function toString(X: BigNum) return String is
   pos: Integer := 0; --position in bignum array
   strPos: Integer := 0; --position in string
   strBigNum: String(1..50); --string representation of X
   begin
      while X(pos) = 0 loop
         pos := pos + 1;
      end loop;
      while pos < Size loop
         strPos := strPos + 1;
         case X(pos) is
            when 0 => strBigNum(strPos) := '0';
            when 1 => strBigNum(strPos) := '1';
            when 2 => strBigNum(strPos) := '2';
            when 3 => strBigNum(strPos) := '3';
            when 4 => strBigNum(strPos) := '4';
            when 5 => strBigNum(strPos) := '5';
            when 6 => strBigNum(strPos) := '6';
            when 7 => strBigNum(strPos) := '7';
            when 8 => strBigNum(strPos) := '8';
            when 9 => strBigNum(strPos) := '9';
            when others => null;
         end case;
         pos := pos + 1;
      end loop;
      return strBigNum(1..strPos);
   end toString;
